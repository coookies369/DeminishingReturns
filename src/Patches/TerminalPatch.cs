...
public class TerminalPatch
{
    ...
    private static void AddDeminishedWarning(ref string modifiedDisplayText, Terminal __instance)
    {
        ...
        if (num > 0)
        {
            ...
            {
                float multiplier = Plugin.moonMultipliers.ContainsKey(__instance.moonsCatalogueList[i].levelID) ? Plugin.moonMultipliers[__instance.moonsCatalogueList[i].levelID] : 1.0f;
